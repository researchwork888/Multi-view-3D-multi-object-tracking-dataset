			...
			for (int k = 0; k < mappings.size(); k++)
			{
				...
				if (foundGtind != gtInd[t].end() && foundStind != stInd[t].end())
				{
					...
					if (world)
					{
                        ...
                        gtz= gtMat[index(rowgt, 8, rowsGT)];
                        gtxaxis= gtMat[index(rowgt, 9, rowsGT)];
                        gtyaxis= gtMat[index(rowgt, 10, rowsGT)];
                        gtzaxis= gtMat[index(rowgt,11, rowsGT)];

						stx = resMat[index(rowres, 6, rowsST)];
						sty = resMat[index(rowres, 7, rowsST)];
                        stz= resMat[index(rowgt, 8, rowsGT)];
                        stxaxis= resMat[index(rowgt, 9, rowsGT)];
                        styaxis= resMat[index(rowgt, 10, rowsGT)];
                        stzaxis= resMat[index(rowgt,11, rowsGT)];
						mexPrintf("%.7lf\n", (gtx));
