...
inline double euclidean(double x1, double y1, double z1,double x2, double y2, double z2)
{
	...
	for (int t = 0; t < Fgt; t++)
	{
			...
			for (int k = 0; k < mappings.size(); k++)
			{
				...
				if (foundGtind != gtInd[t].end() && foundStind != stInd[t].end())
				{
					...
					if (world)
					{
						...
						//mexPrintf("%.7lf %.7lf %.7lf\n", (gtx), (gty), (gtz));
						double dist = euclidean(gtx, gty, gtz, stx, sty, stz);
		...
		for (int k = 0; k < curtracked.size(); k++)
		{
			...
			if (world)
			{
				...
				//mexPrintf("%.7lf %.7lf\n", (gtx), (gty));
				d[t][ct] = euclidean(gtx, gty, gtz, stx, sty, stz);
			}
			else
			{
				double gtleft, gttop, gtwidth, gtheight, stleft, sttop, stwidth, stheight;
