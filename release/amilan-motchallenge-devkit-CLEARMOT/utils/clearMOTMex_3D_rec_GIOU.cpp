...
inline double giou(double x1, double y1,double z1,double xaxis1, double yaxis1,double zaxis1, double x2, double y2,double z2,double xaxis2,double yaxis2,double zaxis2)
{
	// sanity check 
	if ((xaxis1 >= 0) && (yaxis1 >= 0) && (zaxis1 >= 0) && (xaxis2 >= 0 ) && (yaxis2 >= 0) && (zaxis2 >= 0)) {
	...
	vol_2 = rec_area2 * (z2_top - z2_bottom);
		...
		double xym[6], xyM[6];
		xym[0] = min(bottomleft_corner_1x, bottomleft_corner_2x);
		xym[1] = min(bottomleft_corner_1y, bottomleft_corner_2y);
		xym[2] = min(topright_corner_1x, topright_corner_2x);
		xym[3] = min(topright_corner_1y, topright_corner_2y);
		xym[4] = min(z1_bottom, z2_bottom);
		xym[5] = min(z1_top, z2_top);

		xyM[0] = max(bottomleft_corner_1x, bottomleft_corner_2x);
		xyM[1] = max(bottomleft_corner_1y, bottomleft_corner_2y);
		xyM[2] = max(topright_corner_1x, topright_corner_2x);
		xyM[3] = max(topright_corner_1y, topright_corner_2y);
		xyM[4] = max(z1_bottom, z2_bottom);
		xyM[5] = max(z1_top, z2_top);

		double V_CC;
		V_CC = (xyM[2] - xym[0]) * (xyM[3] - xym[1]) * (xyM[5] - xym[4]);
		...
		double Int, VInt;
		if (booll == true) {
			Int = (xym[2] - xyM[0]) * (xym[3] - xyM[1]);

			//if ((xym[3] - xyM[1]) < 0) {
			//	mexPrintf("%.7lf\n", (xym[3] - xyM[1]));
			//}
			VInt = Int * (xym[5] - xyM[4]);
		}
		else {
			VInt = 0;
		}
		double VUnn, V_GIoU, V_IoU, d,jonah;
		VUnn = vol_1 + vol_2 - VInt;
		V_IoU = VInt / VUnn;
		V_GIoU = V_IoU - ((V_CC - VUnn) / V_CC);
		d = 0.5 * (1 - V_GIoU);
		jonah = d;
	...
	for (int t = 0; t < Fgt; t++)
	{
		...
		if (t > 0)
		{
			...
			for (int k = 0; k < mappings.size(); k++)
			{
				...
				if (foundGtind != gtInd[t].end() && foundStind != stInd[t].end())
				{
					...
					if (world)
					{
                        double gtx, gty,gtz,gtxaxis,gtyaxis,gtzaxis;
                        double stx,sty,stz,stxaxis,styaxis,stzaxis;
						int rowgt = gtInd[t][mappings[k]];
						int rowres = stInd[t][M[t - 1][mappings[k]]];
						gtx = gtMat[index(rowgt, 7, rowsGT)];
						gty = gtMat[index(rowgt, 8, rowsGT)];
                        gtz= gtMat[index(rowgt, 9, rowsGT)];
                        gtxaxis= gtMat[index(rowgt, 10, rowsGT)];
                        gtyaxis= gtMat[index(rowgt, 11, rowsGT)];
                        gtzaxis= gtMat[index(rowgt,12, rowsGT)];

						stx = resMat[index(rowres, 7, rowsST)];
						sty = resMat[index(rowres, 8, rowsST)];
                        stz= resMat[index(rowres, 9, rowsST)];
                        stxaxis= resMat[index(rowres, 10, rowsST)];
                        styaxis= resMat[index(rowres, 11, rowsST)];
                        stzaxis= resMat[index(rowres,12, rowsST)];
						//mexPrintf("%.7lf %.7lf %.7lf %.7lf %.7lf %.7lf\n", (gtx),  (gty),  (gtz),  (gtxaxis),  (gtyaxis), (gtzaxis));
						//mexPrintf("%.7lf %.7lf %.7lf %.7lf %.7lf %.7lf\n", (stx), (sty), (stz), (stxaxis), (styaxis), (stzaxis));

						//double dist = euclidean(gtx, gty, stx, sty);
						double dist = giou(gtx, gty, gtz, gtxaxis, gtyaxis, gtzaxis, stx, sty, stz, stxaxis, styaxis, stzaxis);
						//if (dist<0) 
							//mexEvalString("fprintf('error');");
		...
		for (int k = 0; k < curtracked.size(); k++)
		{
			...
			if (world)
			{
				...
				d[t][ct] = giou(gtx, gty, gtz, gtxaxis, gtyaxis, gtzaxis, stx, sty, stz, stxaxis, styaxis, stzaxis);
