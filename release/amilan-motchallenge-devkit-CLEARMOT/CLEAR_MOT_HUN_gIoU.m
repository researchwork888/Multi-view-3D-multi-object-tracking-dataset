function [metrics, metricsInfo, additionalInfo] = CLEAR_MOT_HUN_gIoU(gtMat, resMat, threshold, world)
...
% Evaluate
VERBOSE = false;
[mme, c, fp, m, g, d, alltracked, allfalsepos] = clearMOTMex_3D_rec_GIOU(gtMat, resMat, threshold, world, VERBOSE);
% [mme, c, fp, m, g, d, alltracked, allfalsepos] = clearMOTMex_GIoU(gtMat, resMat, threshold, world, VERBOSE);
% [mme, c, fp, m, g, d, alltracked, allfalsepos] =clearMOTMex_GIoU_testing(gtMat, resMat, threshold, world, VERBOSE);
% [mme, c, fp, m, g, d, alltracked, allfalsepos] = clearMOTMex_IoU(gtMat, resMat, threshold, world, VERBOSE);
...
if world, MOTP =(1-(sum(sum(d))/(threshold*sum(c))) )* 100 ; end
